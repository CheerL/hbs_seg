...
warning(message("map:removing:setltln","SETLTLN","intrinsicToGeographic"))

assert(isequal(size(row),size(col)), ...
    'map:setltln:rowcolSizeMismatch', ...
    'Inconsistent dimensions on row and col inputs.')

validateattributes(row,{'numeric'},{'real'},mfilename,'ROW',3)
validateattributes(col,{'numeric'},{'real'},mfilename,'COL',4)
