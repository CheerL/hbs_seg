...
validateattributes( lat, {'double'}, {'2d','real'}, 'navfix',  'LAT', 1)
validateattributes(long, {'double'}, {'2d','real'}, 'navfix', 'LONG', 2)
validateattributes(  az, {'double'}, {'2d','real'}, 'navfix',   'AZ', 3)
...
if isempty(casetype)
    ...
    validateattributes(casetype, {'numeric'}, {'real'}, 'navfix', 'CASETYPE', 4)
    if ~isequal(size(casetype), size(lat))
...
if ~isempty(drlat)
    validateattributes(drlat, {'double'}, {'scalar','real'}, ...
        'navfix', 'DRLAT', 5)
    validateattributes(drlong, {'double'}, {'scalar','real'}, ...
        'navfix', 'DRLONG', 6)
end
...
for i=1:size(pair,1)
	if casetype(pair(i,1))==1
		if casetype(pair(i,2))==1  % This is the casetype where both objects are azimuths (rhumb lines)
			...
			% sightings are specific to the azimuth given.  Both the wrapTo360 and the wrapTo180
			% tests are done to ensure that 359.999 is counted as close to 0, and -180 as close to 180

			if abs(wrapTo360(azimuth('rh',lat(pair(i,1)),long(pair(i,1)),newlat(i,1),newlong(i,1),'degrees'))-wrapTo360(az(pair(i,1))))>90 && ...
					abs(wrapTo180(azimuth('rh',lat(pair(i,1)),long(pair(i,1)),newlat(i,1),newlong(i,1),'degrees'))-wrapTo180(az(pair(i,1))))>90

				newlat(i,1)=NaN;
				newlong(i,1)=NaN;
			end

			if abs(wrapTo360(azimuth('rh',lat(pair(i,2)),long(pair(i,2)),newlat(i,1),newlong(i,1),'degrees'))-wrapTo360(az(pair(i,2))))>90 && ...
					abs(wrapTo180(azimuth('rh',lat(pair(i,2)),long(pair(i,2)),newlat(i,1),newlong(i,1),'degrees'))-wrapTo180(az(pair(i,2))))>90
			...
			if abs(wrapTo360(azimuth('rh',lat(pair(i,1)),long(pair(i,1)),newlat(i,2),newlong(i,2),'degrees'))-wrapTo360(az(pair(i,1))))>90 && ...
					abs(wrapTo180(azimuth('rh',lat(pair(i,1)),long(pair(i,1)),newlat(i,2),newlong(i,2),'degrees'))-wrapTo180(az(pair(i,1))))>90
	...
	elseif	casetype(pair(i,2))==1   % This is the casetype with the first a range, second an azimuth
		...
		if abs(wrapTo360(azimuth('rh',lat(pair(i,2)),long(pair(i,2)),newlat(i,2),newlong(i,2),'degrees'))-wrapTo360(az(pair(i,2))))>90 && ...
			abs(wrapTo180(azimuth('rh',lat(pair(i,2)),long(pair(i,2)),newlat(i,2),newlong(i,2),'degrees'))-wrapTo180(az(pair(i,2))))>90
			newlat(i,2)=NaN;
			newlong(i,2)=NaN;
		end
