...
%   for a regular data grid F. R is a geographic raster reference object.
%   Its RasterSize property must be consistent with size(F). If the grid
...
%
%   [__] = GRADIENTM(F,REFMAT) and [__] = GRADIENTM(F,REFVEC), where REFMAT
%   is a 3-by-2 referencing matrix and REFVEC is a 1-by-3 referencing
%   vector, will be removed in a future release. Use [__] = GRADIENTM(F,R)
%   instead, where R is a geographic raster reference object.
%
%   [__] = GRADIENTM(LAT,LON,F) computes the gradient of a geolocated
%   data grid.  LAT and LON, the latitudes and longitudes of the
%   geolocation points, are in degrees.
%
%   [__] = GRADIENTM(___,SPHEROID) computes the gradient on the specified
...
% 
%   [__] = GRADIENTM(___,SPHEROID,ANGLEUNIT) specifies the angle
