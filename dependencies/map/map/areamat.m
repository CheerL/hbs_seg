...
%
%   A = AREAMAT(BW,R) returns the surface area covered by the elements of
%   the binary regular data grid BW which contain the value 1 (true).
%   BW might be the result of a logical expression such as BW = (topo > 0).
%   R is a geographic raster reference object. Its RasterSize property must
%   be consistent with size(BW) and its RasterInterpretation must be
%   'cells'. The output A expresses surface area as a fraction of the
%   surface area of the unit sphere (4*pi), so the result ranges from
%   0 to 1.
%
...
%
%   A = AREAMAT(BW,REFVEC,__) and A = AREAMAT(BW,REFMAT,__), where REFVEC
%   is a 1-by-3 referencing vector and REFMAT is a 3-by-2 referencing
%   matrix, will be removed in a future release.  Use A = AREAMAT(BW,R,__)
%   instead, where R is a geographic raster reference object.
%
%   [A,CELLAREA] = AREAMAT(__) returns a vector, CELLAREA, describing
