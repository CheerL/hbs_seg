...
%
%   MAPPROFILE plots a profile of values between waypoints on a displayed
%   regular data grid. MAPPROFILE uses the current object if it is a
%   regular data grid, or the first regular data grid found on the current
%   axes. The grid's zdata is used for the profile. The color data is used
%   in the absence of zdata. The result is displayed in a new figure.
%
%   [ZI,RI,LAT,LON] = MAPPROFILE returns the values of the profile without
%   displaying them. The output ZI contains interpolated values from map
%   along great circles between the waypoints. RI is a vector of associated
%   distances from the first waypoint in units of degrees of arc along the
%   surface. LAT and LON are the corresponding latitudes and longitudes.
%
%   [ZI,RI,LAT,LON] = MAPPROFILE(Z,R,LAT,LON) accepts as input a regular data
%   grid and waypoint vectors. No displayed grid is required. Sets of
%   waypoints may be separated by NaNs into line sequences. The output ranges
%   are measured from the first waypoint within a sequence.  R is a
%   geographic raster reference object. Its RasterSize property must be
%   consistent with size(Z).
%
%   [ZI,RI,LAT,LON] = MAPPROFILE(Z,REFMAT,LAT,LON) and
%   [ZI,RI,LAT,LON] = MAPPROFILE(Z,REFVEC,LAT,LON), where REFMAT is a
%   3-by-2 referencing matrix and REFVEC is a 1-by-3 referencing vector,
%   will be removed in a future release. Use
%   [ZI,RI,LAT,LON] = MAPPROFILE(Z,R,LAT,LON) instead, where R is a
%   geographic raster reference object.
%
%   [ZI,RI,LAT,LON] = MAPPROFILE(__,UNITS) specifies the units of the output
%   ranges along the profile. Valid range units inputs are any distance type
%   recognized by UNITSRATIO. Surface distances are computed using the
%   default radius of the earth. If omitted, 'degrees' are assumed.
%
%   [ZI,RI,LAT,LON] = MAPPROFILE(__,ELLIPSOID) uses the ellipsoid
...
%
%   [ZI,RI,LAT,LON] = MAPPROFILE(__,UNITS,'trackmethod','interpmethod') and
%   [ZI,RI,LAT,LON] = MAPPROFILE(__,ELLIPSOID,'trackmethod','interpmethod')
%   control the interpolation methods used. Valid track methods are 'gc' for
%   great circle tracks between waypoints, and 'rh' for rhumb lines. Valid
%   methods for interpolation within the matrix are 'bilinear' for linear
%   interpolation, 'bicubic' for cubic interpolation, and 'nearest' for
%   nearest neighbor interpolation. If omitted, 'gc' and 'bilinear' are
%   assumed.
%
% See also LOS2
