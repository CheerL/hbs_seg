...
%   mapshow(T,'SymbolSpec',blueRoads)
%
%   Example 2 
%   ---------
...
%                                      {'CLASS',6,'Color','b'},...
%                                      {'Default','Color','k'});
%   mapshow(T,'SymbolSpec',roadColors)
%
%   Example 3 
%   ---------
...
%                                     {'CLASS',[4 6],'LineStyle',':'});
%   mapshow(T,'SymbolSpec',lineStyle)
%
%   Example 4 
%   ---------
...
%   mapshow(T,'SymbolSpec',colorRange)
%
...
%   used as input into other (MathWorks developed) functions. It is
%   expected that users will always use MAKESYMBOLSPEC to create the Symbol
%   Spec, that they won't rely on it as input into their own functions, and
%   therefore the structure can be changed from release to release without
%   breaking backwards compatibility.  It currently has the following form:
%
...
for i=1:length(varargin)
  ...
  if ~isvarname(rule{1})
      error(message( ...
          'map:validate:expectedValidSymbolSpecIdentifier', rule{1}))
  end
  if isdefault(rule) 
