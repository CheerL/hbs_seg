...
%   the line of sight is obscured by terrain.  R is a geographic raster
%   reference object object. Its RasterSize property must be consistent
%   with size(Z). The value of R on output is identical to the value
%   supplied as input. The observer should be located within the
%   latitude-longitude limits of the elevation grid. If the observer is
%   located outside the grid, there is insufficient information to
%   calculate a viewshed.  In this case VIEWSHED issues a warning and sets
%   all elements of VIS to zero.
%
%   [VIS,REFMAT] = VIEWSHED(Z,REFMAT,LAT,LON) and
%   [VIS,REFVEC] = VIEWSHED(Z,REFVEC,LAT,LON), where REFMAT is a 3-by-2
%   referencing matrix and REFVEC is a 1-by-3 referencing vector, will be
%   removed in a future release. Use [VIS,R] = VIEWSHED(Z,R,LAT,LON)
%   instead, where R is a geographic raster reference object.
%
%   VIEWSHED(__,observerAltitude) places the observer at the specified
%   altitude in meters above the surface. This is equivalent to putting the
%   observer on a tower. If omitted, the observer is assumed to be on the
%   surface.
% 
%   VIEWSHED(__,observerAltitude,targetAltitude) checks for visibility of
%   target points a specified distance above the terrain. This is
%   equivalent to putting the target points on towers, but the towers do
%   not obstruct the view. If omitted, the target points are assumed to be
%   on the surface.
% 
%   VIEWSHED(__,observerAltitude,targetAltitude,observerAltitudeOption)
%   controls whether the observer is at a relative or absolute altitude.
%   If observerAltitudeOption is 'AGL', then observerAltitude is in meters
%   above ground level. If observerAltitudeOption is 'MSL',
%   observerAltitude is interpreted as altitude above zero, or mean sea
%   level. If omitted, 'AGL' is assumed.
% 
%   VIEWSHED(__,observerAltitude,targetAltitude, ...
...
%   If the target altitude option is 'AGL', then targetAltitude is in
...
% 
%   VIEWSHED(__,observerAltitude,targetAltitude, observerAltitudeOption, ...
%      targetAltitudeOption, actualRadius,effectiveRadius) assumes a larger
%   radius for propagation of the line of sight. This can account for the
%   curvature of the signal path due to refraction in the atmosphere. For
%   example, radio propagation in the atmosphere is commonly treated as
...
%   where R_e is the radius of the earth. Use Inf for flat earth viewshed
%   calculations. The altitudes, elevations and the radii should be in the
%   same units.
% 
%   Example
%   -------
...
lat2 = intrinsicYToLatitude(R, row);
lon2 = intrinsicXToLongitude(R, col);
vis = false(size(row));
