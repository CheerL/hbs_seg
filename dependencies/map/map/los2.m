...
%   only the data is returned.  R is a geographic raster reference
%   object. Its RasterSize property must be consistent with size(Z).
%
%   VIS = LOS2(Z,REFMAT,__), where REFMAT is a 3-by-2 referencing matrix, and
%   VIS = LOS2(Z,REFVEC,__), where REFVEC is a 1-by-3 referencing vector,
%   will be removed in a future release. Use VIS = LOS2(Z,R,__) instead,
%   where R is a geographic raster reference object.
%
%   VIS = LOS2(__,ALT1) places the first point at the specified altitude in
%   meters above the surface (on a tower, for instance).  This is
%   equivalent to putting the point on a tower.  If omitted, point 1 is
%   assumed to be on the surface.  ALT1 may be either a scalar or a vector
%   with the same length as LAT1, LON1, LAT2, and LON2.
%
%   VIS = LOS2(__,ALT1,ALT2) places both points at a specified altitudes in
%   meters above the surface.  ALT2 may be either a scalar or a vector with
%   the same length as LAT1, LON1, LAT2, and LON2.  If ALT2 is omitted,
%   point 2 is assumed to be on the surface.
%
%   VIS = LOS2(__, ALT1,ALT2,ALT1OPT) controls the interpretation of ALT1
%   as either a relative altitude (ALT1OPT equals 'AGL', the default) or an
%   absolute altitude (ALT1OPT equals 'MSL').  If the altitude option is
%   'AGL', ALT1 is interpreted as the altitude of point 1 in meters above
%   the terrain ("above ground level").  If ALT1OPT is 'MSL', ALT1 is
%   interpreted as altitude above zero ("mean sea level").
%
%   VIS = LOS2(__,ALT1,ALT2,ALT1OPT,ALT2OPT) controls the interpretation
%   ALT2.
%
%   VIS = LOS2(__,ALT1,ALT2,ALT1OPT,ALT2OPT,ACTUALRADIUS) does the
%   visibility calculation on a sphere with the specified radius.  If
%   omitted, the radius of the earth in meters is assumed.  The altitudes,
...
%
%   VIS = LOS2(__,ALT1,ALT2,ALT1OPT,ALT2OPT,ACTUALRADIUS,EFFECTIVERADIUS)
%   assumes a larger radius for propagation of the line of sight.  This can
%   account for the curvature of the signal path due to refraction in the
%   atmosphere.  For example, radio propagation in the atmosphere is
%   commonly treated as straight line propagation on a sphere with 4/3rds
%   the radius of the earth.  In that case the last two arguments would be
%   R_e and 4/3*R_e, where R_e is the radius of the earth.  Use Inf as the
%   effective radius for flat earth visibility calculations.  The
%   altitudes, elevations and the radii should be in the same units.
%
%   [VIS,VISPROFILE,DIST,H,LATTRK,LONTRK] = LOS2(__), for scalar inputs
%   (LAT1, LON1, etc.), returns vectors of points along the path between
%   the two points.  VISPROFILE is a logical vector containing true
...
%
%   LOS2(__), with no output arguments, displays the visibility profile
