...
narginchk(1,4)
warning(message("map:removing:pixcenters","PIXCENTERS","worldGrid","geographicGrid"))
...
%--------------------------------------------------------------------------
function [x, y] = pixcenters_refmat(refmat, height, width, gridrequest)
% Use referencing matrix input.

if (gridrequest || (refmat(1,1) ~= 0) || (refmat(2,2) ~= 0))
    [r,c] = ndgrid(1:height,1:width);
    [x,y] = map.internal.refmatApplyForward(refmat,r,c);
else
    [x,~] = map.internal.refmatApplyForward(refmat,ones(1,width),1:width);
    [~,y] = map.internal.refmatApplyForward(refmat,1:height,ones(1,height));
end
