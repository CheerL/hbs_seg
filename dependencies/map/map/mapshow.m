...
%
%   MAPSHOW(X,Y) or MAPSHOW(X,Y, ___, 'DisplayType', DISPLAYTYPE, ___)
...
%   'line'.
%
%   MAPSHOW(X,Y,Z, ___, 'DisplayType', DISPLAYTYPE, ___) displays a
...
%   'contour'.
%
%   MAPSHOW(Z,R, ___, 'DisplayType', DISPLAYTYPE, ___) displays a regular
...
%   MAPSHOW(X,Y,A,CMAP) 
%   MAPSHOW(X,Y,RGB)
%   MAPSHOW(___, 'DisplayType', DISPLAYTYPE, ___) displays a geolocated
...
%   MAPSHOW(RGB,R) 
%   MAPSHOW(A,CMAP,R) 
%   MAPSHOW(___, 'DisplayType', DISPLAYTYPE, ___) displays an image
...
%   'image'.
%
%   MAPSHOW(S) displays the vector geospatial features stored in S as
%   points, multipoints, lines, multilines, polygons, or multipolygons
%   according to the geometry of S. S may be either a geospatial table
%   containing mappointshape, maplineshape, or mappolyshape objects, a
%   mappoint vector, a mapshape vector, or a mapstruct (with 'X' and 'Y'
%   coordinate fields). The coordinate values are used directly to plot
%   features in map coordinates. If S is either a geopoint vector, a
...
%   issued. If S is a geospatial table containing geopointshape,
%   geolineshape, or geopolyshape objects an error is issued.
%
%   MAPSHOW(S, ___, 'SymbolSpec', SYMSPEC, ___) symbolizes the vector
%   geographic features stored in S. SYMSPEC specifies the symbolization
%   rules used for the vector data through a structure returned by
%   MAKESYMBOLSPEC.
%
...
%
%   MAPSHOW(AX, ___) sets the axes parent to AX. This is equivalent to 
%   MAPSHOW(___, 'Parent', AX, ___)
%
%   H = MAPSHOW(___) returns a handle to a MATLAB graphics object or, in
...
%
%   MAPSHOW(___, Name, Value) specifies name-value pairs that modify the
...
%   T = readgeotable('boston_roads.shp');
%   figure
%   mapshow(T,'LineStyle',':');
%
%   Example 3 
...
%   T = readgeotable('concord_hydro_area.shp');
%   mapshow(T(14,:),'FaceColor',[0.3 0.5 1],'EdgeColor','black')
%
...
%   filename = fullfile(matlabroot,'examples','map','data','sdts','9129CATD.DDF');
...
%   demcmap(Z_W)
%
%   See also GEOSHOW, MAKESYMBOLSPEC
